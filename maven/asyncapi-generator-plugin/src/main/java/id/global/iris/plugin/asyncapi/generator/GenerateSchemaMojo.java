...
import id.global.iris.asyncapi.runtime.client.ApicurioClient;
import id.global.iris.asyncapi.runtime.io.AsyncApiSerializer;
import id.global.iris.asyncapi.runtime.io.Format;
import id.global.iris.asyncapi.runtime.scanner.GidAnnotationScanner;
...
public class GenerateSchemaMojo extends AbstractMojo {
    ...
    private AaiDocument generateSchema(IndexView index) throws IOException, ClassNotFoundException, MojoExecutionException {
        ...
        GidAnnotationScanner scanner = new GidAnnotationScanner(asyncApiConfig, index, classLoader, mavenProject.getName(),
    ...
    private void write(AaiDocument schema) throws MojoExecutionException {
        try {
            if (outputDirectory == null) {
                ...
                getLog().info(getObjectMapper().writeValueAsString(schema));
            } else {
                ...
                writeSchemaFile(directory, schemaFilename + ".yaml", schema, getYamlObjectMapper());
                writeSchemaFile(directory, schemaFilename + ".json", schema, getObjectMapper());
