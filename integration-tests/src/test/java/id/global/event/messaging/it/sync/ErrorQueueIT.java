...
public class ErrorQueueIT extends AbstractIntegrationTest {
    ...
    void corruptedMessage() throws Exception {
        ...
        assertThat(errorMessage.name(), is(MessagingError.ERR_BAD_REQUEST.getName()));
    ...
    void serverException() throws Exception {
        ...
        final var errorMessage = getErrorResponse(5);
        assertThat(errorMessage, is(notNullValue()));
        assertThat(errorMessage.name(), is(MessagingError.ERR_SERVER_ERROR.name()));
    ...
    public static class ErrorQueueService {
        ...
        public void handle(BadRequestMessage message) {
            throw new BadMessageException(MessagingError.ERR_BAD_REQUEST, "Unable to process message. Message corrupted.");
        }

        @MessageHandler
        public void handle(ServerErrorMessage message) {
            throw new ServerException(MessagingError.ERR_SERVER_ERROR, "Internal service error.", true);
        }
    }
